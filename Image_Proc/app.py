...
[List any significant vitamins and minerals]"""
    ...
    def post(self):
        try:
            # Check for image or text input
            data = request.get_json() if request.content_type == 'application/json' else None
            question = data.get('question', '').strip() if data else ''
            image = request.files.get('image')

            if image:
                # Process the uploaded image using extract.py
                image_bytes = image.read()
                mime_type = image.mimetype
                json_data = extract.get_food_items_from_image(image_bytes, mime_type)
                
                # Debug print the JSON data
                print("JSON Data from extract.py:", json_data)

                # Parse the JSON data
                food_items = json.loads(json_data).get('food_items', [])
                
                if not food_items:
                    return jsonify({"error": "No food items detected in the image."}), 400

                # Use the food items to process the meal query
                components = ', '.join(food_items)
                meal_response = self.process_meal_query(f"eating {components}")
